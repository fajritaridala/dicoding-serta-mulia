...
const predictClassification = async (model, image) => {
  try {
    const tensor = tf.node // .node -> menangani proses inferensi data
      .decodeJpeg(image) // melakukan decode terhadap input data baru
      .resizeNearestNeighbor([224, 224]) //melakukan resize gambar menggunakan algoritma nearest neighbor
      .expandDims() // menambah dimensi gambar
      .toFloat(); // mengubah seluruh data yang diproses menjadi float
